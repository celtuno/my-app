...
export const Activities: React.FC<Props> = ({ name }) => {
  ...
  const GetData = async () => {
    const dataFound = await PlannerHandler.GetDBAllIemsSnapshot(
      DataType.Planner,
      ""
    );
    if (dataFound) {
  ...
  useEffect(() => {
    if (!snapshotFound && days.length <= 0) {
      GetData();
      console.log("Activity SnapEffect");
    }
  });

  //   useEffect(() => {
  //     if (!user && !authuser) GetUserData();
  //     console.log("Activity: UserEffect");
  //   });
        ...
        {snapshotFound && authuser ? (
          days?.map((dayItem, dayIndex) =>
            dayItem ? (
              ...
              >
                <React.Fragment>
                  <Text key={dayIndex} style={styles.subheading}>
                    {dayItem.Name}
                  </Text>
                </React.Fragment>
