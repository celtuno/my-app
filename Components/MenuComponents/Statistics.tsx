...
export const Statistics: React.FC<Props> = ({
  ...
  const [activityCount, setactivityCount] = useState<number | null | undefined>(
    null
  );
  const [presetCount, setpresetCount] = useState<number | null | undefined>(
    null
  );
  ...
  useEffect(() => {
    if (!activityCount && !presetCount && !userCount) GetData();
  });
  const GetData = async () => {
    await PlannerHandler.GetDBSnapshotCount(DataType.Planner).then(
      (countData) => {
        console.log(countData);
        setactivityCount(countData.activityCount);
      }
    );
    await PlannerHandler.GetDBSnapshotCount(DataType.Presets).then(
      (countData) => {
        console.log(countData);
