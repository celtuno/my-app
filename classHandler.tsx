...
export const Users: React.FC<Props> = ({ name, authuser }) => {
  ...
  const GetData = async () => {
    const tmp = await PlannerHandler.GetDBAllIemsSnapshot(DataType.User, "")
      .then((dataFound) => {
          ...
          users?.map((userItem, userIndex) =>
            userItem ? (
              <View
                ...
                style={[
                  {
                    backgroundColor: userItem.Config.color1
                      ? userItem.Config.color1
                      : "#05050310",
                    borderRadius: 4,
                    margin: 15,
                    padding: 15,
